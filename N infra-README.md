...
| Metric | Value | Notes |
|---|---|---|
| ACE-Step first-run weight download | ____ GB / ____ min | one-time |
| 60s instrumental gen (turbo + 1.7B LM, batch 2) | ____ s wall-clock | drives queue math |
| Peak memory during generation | ____ GB | must fit under ~24GB w/ headroom |
| Master (2-pass loudnorm + length lock) | ____ s | ffmpeg CPU |
| librosa landmark analysis | ____ s | per take |
| Full job (claim → 2 takes → mastered → uploaded) | ____ s | end-to-end |
| Client Stockfish full-game sweep (mid phone) | ____ s | ~96 plies, depth ~18–22 |
| Sustained throughput | ____ soundtracks/hour | = 3600 / full-job-time |
