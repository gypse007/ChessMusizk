...
def pick_bpm(p: StyleProfile, pace_median_s: Optional[float]) -> int:
    ...
    if pace_median_s is None:
        return (lo + hi) // 2
    t = max(0.0, min(1.0, (8.0 - pace_median_s) / 7.0))
...
def event_graph_to_spec(
    ...
    if time_profile:
        category = time_profile.speed
    elif graph.timeCategory:
        category = graph.timeCategory
    else:
        tc = parse_timecontrol(graph.timeControl)
        category = classify_speed(tc)
    ...
    negative_prompt = get_negative_prompt_for_style(category)
    ...
    details = ", ".join(caption_parts)
    caption = f"{details}, {negative_prompt}"
